...
export function CarouselComponent2(){
  ...
  return(
    ...
    >
      <CarouselContent className="flex ">
        {news.map((article, index) => (
          <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3 bg-transparent">
            <div className="p-3 bg-transparent border-2 border-black rounded-lg flex flex-col" style={{ height: '90px' }}>
      ...
      <CarouselPrevious 
        className="p-2 mt-2 lg:mt-1 absolute -left-2 top-1/2 transform -translate-y-1/2 text-white p-2 bg-gray-200 hover:bg-gray-400 rounded-full opacity-70 md:-left-4 transition-all z-10" 
      />
      <CarouselNext
        ref={nextButtonRef}
        className="p-2 mt-2 lg:mt-1 absolute -right-2 top-1/2 transform -translate-y-1/2 text-whites p-2 bg-gray-200 hover:bg-gray-400 rounded-full opacity-70 md:-right-4 transition-all z-10" 
      />
