...
export function CarouselComponent2(){
  ...
  return(
    ...
    >
          ...
          <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3 bg-transparent">
            <div className="p-3 bg-transparent border-2 border-gray-300 rounded-lg flex flex-col" style={{ height: '90px' }}>
              <div className="flex justify-between items-center w-full">
                <span className="text-lg text-white font-semibold max-w-[80%] overflow-hidden text-ellipsis">
                  {article.title}
                </span>
                <button
      ...
      <CarouselPrevious 
        className="p-2 mt-2 lg:mt-1 absolute left-0 top-1/2 transform -translate-y-1/2 text-black p-2 bg-gray-200 hover:bg-gray-400 rounded-full opacity-70 md:-left-4 transition-all z-10" 
      />
      <CarouselNext
        ref={nextButtonRef}
        className="p-2 mt-2 lg:mt-1 absolute right-0 top-1/2 transform -translate-y-1/2 text-black p-2 bg-gray-200 hover:bg-gray-400 rounded-full opacity-70 md:-right-4 transition-all z-10" 
      />
