...
export function Semdropdown({branch}){
  ...
  <div className="flex justify-center">
    <DropdownMenu>
      <DropdownMenuTrigger className="font-bold text-white text-2xl lg:flex focus:outline-none hover:text-gray-500 border-2 border-white shadow-md transition-all p-1 rounded-lg">
      ...
      <DropdownMenuContent className="w-full max-h-60 bg-slate-900 rounded-md shadow-lg border border-white overflow-y-auto p-2 min-w-[200px]">
        {branch === "aids" ? (
          ...
          .map((sem, index, array) => ( 
            <DropdownMenuItem 
              key={sem.number} 
              className="p-3 relative text-white font-bold text-xl transition-all duration-200 cursor-pointer"
              onClick={() => setSeminfo(sem.number)}
            >
              <div className="mx-auto">
                {sem.number}
